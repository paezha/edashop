...
We'll create a dictionary abc with information about municipalities in Greater ABC Paulista, São Paulo, Brazil, from the [2022 Demographic Census](https://cidades.ibge.gov.br/). The first column includes the names of the municipalities; the second column contains the population; the third is the average salary in terms of minimum wage; the next is the number of workers; and finally, there is a column with the area of the municipality in square kilometers. 

```{python}
abc = {'Name': ["Santo André","São Bernardo Do Campo","São Caetano Do Sul",         "Diadema","Rio Grande Da Serra","Ribeirão Pires","Mauá"],
...
```{python}
df.loc[df['Name'] == "Santo André", 'Workers'].values[0] + df.loc[df['Name'] == "São Bernardo Do Campo", 'Workers'].values[0]
```
...
2. Load the São Paulo Social Vulnerability Index (IVSP) dataset at census tract level (included in the package [{edashop}](https://paezha.github.io/edashop/)). To make it accessible in Python, we first load it in R and then access it via reticulate. 
